        ...
        <div style={{
          ...
          {!isReviewed ? (
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              <button onClick={(e) => handleReview(e, 1)} style={{ padding: '8px 12px', fontSize: 11, backgroundColor: '#ff4d4d', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 700 }}>HARD</button>
              <button onClick={(e) => handleReview(e, 3)} style={{ padding: '8px 12px', fontSize: 11, backgroundColor: '#ffb800', color: '#000', border: 'none', borderRadius: 8, fontWeight: 700 }}>GOOD</button>
              <button onClick={(e) => handleReview(e, 5)} style={{ padding: '8px 12px', fontSize: 11, backgroundColor: '#00ff00', color: '#000', border: 'none', borderRadius: 8, fontWeight: 700 }}>EASY</button>
            </div>
          ) : (
            <div style={{ marginTop: 20, fontSize: 12, color: accentColor, fontWeight: 700 }}>✅ MASTERED</div>
          )}
    ...
    <div style={{ backgroundColor: 'rgba(34, 34, 34, 0.84)', border: `1px solid ${accentColor}`, borderRadius: 30, padding: 30 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {question.options.map((opt, idx) => {
          ...
          if (selectedOpt !== null) {
            if (opt === question.correctAnswer) { bgColor = 'rgba(0, 255, 0, 0.15)'; borderColor = '#00ff00'; }
            else if (opt === selectedOpt) { bgColor = 'rgba(255, 0, 0, 0.15)'; borderColor = '#ff0000'; }
          }
          return (
            <button key={idx} onClick={() => handleSelect(opt)} style={{ textAlign: 'left', padding: '16px 20px', borderRadius: 16, backgroundColor: bgColor, border: `1px solid ${borderColor}`, color: '#F5F5F5', cursor: selectedOpt === null ? 'pointer' : 'default', transition: 'all 0.2s' }}>{opt}</button>
          );
        })}
      </div>
      {showExplanation && (
        <div style={{ marginTop: 24, padding: 20, backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: 20, border: '1px solid rgba(255,255,255,0.1)' }}>
          ...
          {currentIdx < testData.questions.length - 1 && (
            <button onClick={handleNext} style={{ marginTop: 16, padding: '10px 24px', backgroundColor: accentColor, color: '#000', border: 'none', borderRadius: 12, fontWeight: 700, cursor: 'pointer' }}>Next Question →</button>
          )}
        </div>
      )}
    </div>
      ...
      <div style={{ marginTop: 30, display: 'flex', flexDirection: 'column', gap: 20 }}>
        {data.keyTakeaways.map((item, idx) => (
          <div key={idx} style={{ padding: 20, backgroundColor: 'rgba(0,0,0,0.2)', borderRadius: 24, border: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            {item.mnemonic && <div style={{ padding: '8px 16px', backgroundColor: 'rgba(179, 255, 0, 0.1)', borderRadius: 12, fontSize: 14, fontStyle: 'italic', display: 'inline-block' }}>💡 Mnemonic: {item.mnemonic}</div>}
    ...
    <div style={{ backgroundColor: 'rgba(34, 34, 34, 0.84)', border: `1px solid ${accentColor}`, borderRadius: 30, padding: 30 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {planData.tasks.sort((a, b) => a.order - b.order).map((task, idx) => (
          <div key={idx} style={{ display: 'flex', gap: 20, padding: 20, backgroundColor: 'rgba(255, 255, 255, 0.03)', borderRadius: 20, border: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', backgroundColor: accentColor, color: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 18, flexShrink: 0 }}>{task.order}</div>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8, flexWrap: 'wrap', gap: 10 }}>
                ...
                <div style={{ fontSize: 12, padding: '4px 12px', borderRadius: 12, backgroundColor: task.priority.includes('Critical') ? 'rgba(255,0,0,0.2)' : 'rgba(179,255,0,0.1)', color: task.priority.includes('Critical') ? '#ff4d4d' : accentColor, border: `1px solid ${task.priority.includes('Critical') ? '#ff4d4d' : accentColor}` }}>{task.priority}</div>
              </div>
...
function ScrollReveal({ children, isLocked }) {
  ...
  return (
    <div ref={domRef} style={{ opacity: isVisible ? 1 : 0, transform: isVisible ? 'translateY(0)' : 'translateY(40px)', transition: 'opacity 0.8s ease-out, transform 0.8s ease-out', marginInline: 'auto', width: '100%', marginBottom: isLocked ? 20 : 40, position: 'relative', minHeight: isLocked ? '120px' : 'auto' }}>
      <div style={{ position: 'relative', width: '100%' }}>
        <div style={{ filter: isLocked ? 'blur(10px) grayscale(100%)' : 'none', pointerEvents: isLocked ? 'none' : 'auto', transition: 'filter 0.5s ease', opacity: isLocked ? 0.4 : 1 }}>{children}</div>
        {isLocked && (
          <div style={{ position: 'absolute', inset: 0, zIndex: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 50, pointerEvents: 'none' }}>
            <div style={{ backgroundColor: 'rgba(0,0,0,0.85)', color: '#B3FF00', padding: '15px 30px', borderRadius: 30, boxShadow: '0 10px 40px rgba(0,0,0,0.8), 0 0 20px rgba(179, 255, 0, 0.2)', fontWeight: 700, fontSize: 20, border: '1px solid rgba(179, 255, 0, 0.5)', display: 'flex', alignItems: 'center', gap: 12, pointerEvents: 'none', transform: 'translateY(-10px)' }}>
...
const Dashboard = () => {
  ...
  const [workspaceId, setWorkspaceId] = useState(currentUser?.uid || 'user_123');
  ...
  useEffect(() => {
    if (urlSessionId && currentUser?.uid) {
      const fetchSession = async () => {
        try {
          const res = await fetch(`${apiBase}/api/sessions/${encodeURIComponent(currentUser.uid)}`);
          const data = await parseResponse(res);
          if (data.ok && data.sessions) {
            const session = data.sessions.find(s => s.sessionId === urlSessionId);
            if (session) {
              setUploadId(session.sessionId);
              setSessionId(session.sessionId);
              setWorkspaceId(session.workspaceId || currentUser.uid);
  ...
  }, [urlSessionId, currentUser, apiBase]);

  useEffect(() => {
    let interval;
    if (isEmergencyActive && isVaultOpen) {
      interval = setInterval(() => { setSurvivalSeconds(prev => prev + 1); }, 1000);
  ...
  }, [isFetchingMore, uploadId, rawNotes, currentUser, flashcards, apiBase, syllabusAnalysis, examAnalysis, workspaceId, sessionId]);
  ...
  async function handleOcrImage(target) {
    ...
    if (!fileToUse) {
      if (target === 'syllabus') setSyllabusImageError('Please select a syllabus image.');
      else setNotesImageError('Please select a past papers image.');
      return;
    }
    if (target === 'syllabus') { setSyllabusImageLoading(true); setSyllabusImageError(''); }
    else { setNotesImageLoading(true); setNotesImageError(''); }
    try {
      const formData = new FormData();
      formData.append('image', fileToUse);
      const res = await fetch(`${apiBase}/api/ocr/image`, { method: 'POST', body: formData });
      const data = await parseResponse(res);
      ...
      if (target === 'syllabus') setSyllabusImageError(err.message || String(err));
      else setNotesImageError(err.message || String(err));
    } finally {
      if (target === 'syllabus') setSyllabusImageLoading(false);
      else setNotesImageLoading(false);
    }
  }
  ...
  async function handleExtract(e) {
    ...
    setExtractLoading(true); setError(''); setNotice(''); setResult('');
    ...
    try {
      ...
      const data = await parseResponse(res);
      if (!data.ok) throw new Error(data.error);
      setUploadId(data.uploadId); setRawNotes(data.rawText); setResult(data.rawText); setResultSource('extracted'); setGeneratedData(null); setNotice(data.warning || '');
  ...
  async function handleAnalyze(e) {
    ...
    setAnalyzeLoading(true); setError(''); setResult('');
    try {
      if (sessionId) {
        await fetch(`${apiBase}/api/sessions/${encodeURIComponent(currentUser.uid)}/${encodeURIComponent(sessionId)}/append`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ newSyllabusText: syllabusText, newPastPapersText: pastPapersText.trim() || null })
        });
      }
      const res = await fetch(`${apiBase}/api/analyze`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ rawNotes, syllabusText, pastPapersText: pastPapersText.trim() || null }),
      });
      const data = await parseResponse(res);
      if (!data.ok) throw new Error(data.error);
      setSyllabusAnalysis(data.syllabusAnalysis); setExamAnalysis(data.examAnalysis); setResult(JSON.stringify(data, null, 2)); setResultSource('analyzed'); setGeneratedData(null);
    } catch (err) { setError(err.message || String(err)); }
  ...
  async function handleGenerateOutput(e) {
    e.preventDefault();
    if (!currentUser?.uid) { setError('Please sign in to continue.'); return; }
    if (!uploadId) { setError('Please extract content first to get an uploadId.'); return; }
    if (!rawNotes.trim()) { setError('Please provide raw notes for generation.'); return; }
    setGenerateLoading(true); setError(''); setResult('');
    try {
      const res = await fetch(`${apiBase}/api/generate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ uploadId, requestType, rawNotes, syllabusAnalysis, examAnalysis, specificTopic: specificTopic.trim() ? specificTopic.trim() : null, workspaceId: workspaceId.trim(), userId: currentUser.uid }),
      });
      const data = await parseResponse(res);
      if (!data.ok) throw new Error(data.error);
      setResult(JSON.stringify(data.data, null, 2)); setResultSource(`generated (from ${data.source})`); setGeneratedData(data.data);
      if (requestType === 'flashcards' && data.data.flashcards) {
        const newCards = data.data.flashcards.map(c => ({
          ...c, id: typeof crypto.randomUUID === 'function' ? crypto.randomUUID() : Math.random().toString(36).slice(2)
        }));
        ...
        } catch (onboardErr) { console.warn("Failed to onboard flashcards for analytics", onboardErr); }
      } else { setFlashcards([]); }
    } catch (err) { setError(err.message || String(err)); }
  ...
  async function handleFetchHeatmap(e) {
    ...
    setHeatmapLoading(true); setHeatmapError(''); setHeatmapResult(null);
    try {
  ...
  async function handleSurvivalModalSubmit(hrs) {
    ...
    if (trimmedWorkspaceId && !Number.isNaN(hoursValue)) {
      setSurvivalLoading(true); setSurvivalError(''); setSurvivalPlan(null); setIsVaultOpen(false); setIsEmergencyActive(true); setIsSyncing(true); setShowSurvivalModal(false);
      try {
        const res = await fetch(`${apiBase}/api/survival/triage`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ workspaceId: trimmedWorkspaceId, hoursRemaining: hoursValue, uploadId: uploadId }),
        });
        const data = await parseResponse(res);
        if (!data.ok) throw new Error(data.error);
        setSurvivalPlan(data.data);
        setTimeout(() => { setIsSyncing(false); setIsVaultOpen(true); }, 5000); 
      } catch (err) { setSurvivalError(err.message || String(err)); setIsEmergencyActive(false); setIsSyncing(false); }
      finally { setSurvivalLoading(false); }
    }
  }

  function handleStopSurvival() { setIsEmergencyActive(false); setIsVaultOpen(false); setIsSyncing(false); setSurvivalPlan(null); setSurvivalSeconds(0); }

  const heatmapStatusStyles = { Green: { backgroundColor: '#d6f5d6', color: '#1f7a1f' }, Yellow: { backgroundColor: '#fff3bf', color: '#8a6d00' }, Red: { backgroundColor: '#ffe3e3', color: '#a61e1e' } };
  const accentColor = '#B3FF00';
  const translucentPanelStyle = { marginBottom: 36, padding: '30px 28px 26px', border: '1px solid rgba(179, 255, 0, 0.75)', borderRadius: 50, backgroundColor: 'rgba(34, 34, 34, 0.84)', backdropFilter: 'blur(10px)', boxShadow: '0 0 0 1px rgba(179, 255, 0, 0.34), 0 0 22px rgba(179, 255, 0, 0.16), 0 18px 40px rgba(0, 0, 0, 0.22)' };
  const glassyInputStyle = { width: '100%', minHeight: 56, padding: '16px 18px', boxSizing: 'border-box', border: '1px solid rgba(255, 255, 255, 0.14)', borderRadius: 20, background: 'rgba(71, 71, 71, 0.72)', color: '#F5F5F5', backdropFilter: 'blur(10px)', boxShadow: 'inset 0 1px 0 rgba(255, 255, 255, 0.08), 0 10px 24px rgba(0, 0, 0, 0.18)' };
  const glassyTextAreaStyle = { ...glassyInputStyle, minHeight: 148, resize: 'vertical' };
  const glassySelectStyle = { ...glassyInputStyle, width: 'auto', minWidth: 180, padding: '12px 16px' };
  const getActionButtonStyle = (disabled = false) => ({ padding: '14px 22px', cursor: disabled ? 'not-allowed' : 'pointer', fontWeight: 700, backgroundColor: accentColor, color: '#000000', border: 'none', borderRadius: 18, opacity: disabled ? 0.45 : 1, boxShadow: disabled ? 'none' : '0 10px 24px rgba(179, 255, 0, 0.22)' });
  const uploadPickerButtonStyle = { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minHeight: 52, padding: '0 20px', backgroundColor: accentColor, color: '#000000', borderRadius: 18, fontWeight: 700, cursor: 'pointer', boxShadow: '0 10px 24px rgba(179, 255, 0, 0.22)' };
  const fileCarouselStyle = { display: 'flex', gap: 14, overflowX: 'auto', paddingBottom: 8, WebkitOverflowScrolling: 'touch', scrollSnapType: 'x proximity' };
  const fileCardStyle = { minWidth: 220, maxWidth: 220, minHeight: 190, padding: '18px 18px 16px', borderRadius: 22, border: '1px solid rgba(179, 255, 0, 0.45)', background: 'rgba(71, 71, 71, 0.78)', color: '#F5F5F5', boxSizing: 'border-box', scrollSnapAlign: 'start', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: 'inset 0 1px 0 rgba(255, 255, 255, 0.06), 0 12px 26px rgba(0, 0, 0, 0.2)' };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: 'transparent' }}>
      ...
      <div className="app-header" style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '25px 50px' }}>
        ...
        <div className="header-centerpiece" style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', zIndex: 10 }}>
          {isEmergencyActive && isVaultOpen ? (
            <div className="header-timer" style={{ position: 'static', transform: 'none', padding: '15px 45px', borderRadius: '30px', fontSize: '32px', boxShadow: '0 0 30px rgba(255, 77, 77, 0.3)' }}>
              <span style={{ fontSize: 16, opacity: 0.8, letterSpacing: 2, fontWeight: 900 }}>SURVIVAL CLOCK</span>
              {formatTime(survivalSeconds)}
            </div>
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', gap: 15, opacity: 0.6 }}>
              <div className="pulse" style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: '#B3FF00' }} />
              <span style={{ color: '#B3FF00', fontWeight: 800, fontSize: 13, letterSpacing: 4 }}>SYSTEM STANDBY</span>
            </div>
          )}
        </div>
      ...
      <div className="tab-content" style={{ flex: 1, overflowY: activeTab === 0 ? 'auto' : 'hidden' }}>
              ...
              <section style={{ ...translucentPanelStyle, minHeight: 280, paddingTop: 55, paddingBottom: 15 }}>
                ...
                <form onSubmit={handleExtract}>
                  <div style={{ marginBottom: 18, maxWidth: 980, marginInline: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, flexWrap: 'wrap' }}>
                    <div style={{ flex: '1 1 640px', minWidth: 320 }}><input type="text" placeholder="YouTube URL (or leave blank for file upload)" value={youtubeUrl} onChange={(e) => setYoutubeUrl(e.target.value)} style={glassyInputStyle} /></div>
                    <label style={{ display: 'inline-flex', alignItems: 'center', gap: 10, minHeight: 56, padding: '0 18px', borderRadius: 18, background: 'rgba(71, 71, 71, 0.72)', border: '1px solid rgba(255, 255, 255, 0.14)', color: '#E8E8E8', fontWeight: 600, whiteSpace: 'nowrap' }}><input type="checkbox" checked={forceWhisper} onChange={(e) => setForceWhisper(e.target.checked)} />Force Groq Whisper</label>
                  </div>
                  <div style={{ marginBottom: 22, width: '100%', textAlign: 'center' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginBottom: 14, flexWrap: 'wrap' }}><label htmlFor="study-material-upload" style={uploadPickerButtonStyle}>Choose Files</label><span style={{ color: '#D6D6D6', fontWeight: 600 }}>{files.length > 0 ? `${files.length} file${files.length === 1 ? '' : 's'} ready` : 'Upload PDF, DOCX, or PPTX'}</span></div>
                    {files.length > 0 && (<div className="file-carousel" style={fileCarouselStyle}>{files.map((selectedFile, index) => (<div key={`${selectedFile.name}-${index}`} style={fileCardStyle}>{(() => { const fileType = getFileTypeMeta(selectedFile.name); return (<><div style={{ width: 86, height: 86, marginInline: 'auto', borderRadius: 24, background: `linear-gradient(135deg, ${fileType.color} 0%, ${fileType.color} 62%, ${fileType.accent} 62%, ${fileType.accent} 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FFFFFF', fontWeight: 800, fontSize: fileType.label.length > 3 ? 18 : 24, letterSpacing: 0.6, boxShadow: `0 10px 24px ${fileType.accent}` }}>{fileType.label}</div><div style={{ color: accentColor, fontWeight: 700, fontSize: 13, textAlign: 'center', marginTop: 14 }}>Document {index + 1}</div></>); })()}<div style={{ fontWeight: 600, lineHeight: 1.35, wordBreak: 'break-word', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden', textAlign: 'center' }}>{selectedFile.name}</div><div style={{ color: '#BDBDBD', fontSize: 12, textAlign: 'center' }}>{Math.max(1, Math.round(selectedFile.size / 1024))} KB</div></div>))}</div>)}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}><button type="submit" disabled={extractLoading || (!youtubeUrl && files.length === 0)} style={getActionButtonStyle(extractLoading || (!youtubeUrl && files.length === 0))}>{extractLoading ? '⏳ Extracting…' : '📤 Extract Content'}</button><LoadingProgressBar loading={extractLoading} label="Extracting Content" />{uploadId && <p style={{ color: 'green', marginTop: 8, textAlign: 'center' }}>✅ Extracted! Upload ID: {uploadId.slice(0, 8)}...</p>}</div>
                </form>
              </section>
            </div>
                ...
                <form onSubmit={handleAnalyze}>
                  <div style={{ display: 'flex', gap: 20, marginBottom: 20, flexWrap: 'wrap' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}><input id="syllabus-image-upload" type="file" accept=".png,.jpg,.jpeg,.webp" onChange={(e) => setSyllabusImage(e.target.files?.[0] || null)} style={{ display: 'none' }} /><label htmlFor="syllabus-image-upload" style={uploadPickerButtonStyle}>{syllabusImage ? '✅ Selected' : '🖼️ Choose Image'}</label><button type="button" onClick={() => handleOcrImage('syllabus')} disabled={syllabusImageLoading || !syllabusImage} style={getActionButtonStyle(syllabusImageLoading || !syllabusImage)}>{syllabusImageLoading ? '⏳ Extracting…' : '📄 Extract from image'}</button>{syllabusImage && <span style={{ fontSize: 12, opacity: 0.7, maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{syllabusImage.name}</span>}</div>
                      <LoadingProgressBar loading={syllabusImageLoading} label="OCR Syllabus" />{syllabusImageError && <div style={{ color: 'crimson', marginTop: 8 }}>{syllabusImageError}</div>}
                    </div>
                    <div style={{ flex: 1 }}>
                      <label style={{ display: 'block', marginBottom: 10 }}>Past Papers Image (optional)</label>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap' }}><input id="notes-image-upload" type="file" accept=".png,.jpg,.jpeg,.webp" onChange={(e) => setNotesImage(e.target.files?.[0] || null)} style={{ display: 'none' }} /><label htmlFor="notes-image-upload" style={uploadPickerButtonStyle}>{notesImage ? '✅ Selected' : '🖼️ Choose Image'}</label><button type="button" onClick={() => handleOcrImage('notes')} disabled={notesImageLoading || !notesImage} style={getActionButtonStyle(notesImageLoading || !notesImage)}>{notesImageLoading ? '⏳ Extracting…' : '📄 Extract from image'}</button>{notesImage && <span style={{ fontSize: 12, opacity: 0.7, maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{notesImage.name}</span>}</div>
                      <LoadingProgressBar loading={notesImageLoading} label="OCR Past Papers" />{notesImageError && <div style={{ color: 'crimson', marginTop: 8 }}>{notesImageError}</div>}
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 20, marginBottom: 18, flexWrap: 'wrap' }}><textarea placeholder="Paste syllabus text here (required)" value={syllabusText} onChange={(e) => setSyllabusText(e.target.value)} rows={4} style={{ ...glassyTextAreaStyle, flex: 1 }} /><textarea placeholder="Paste past exam papers (optional)" value={pastPapersText} onChange={(e) => setPastPapersText(e.target.value)} rows={4} style={{ ...glassyTextAreaStyle, flex: 1 }} /></div>
                  <button type="submit" disabled={analyzeLoading || !rawNotes.trim() || !syllabusText.trim()} style={getActionButtonStyle(analyzeLoading || !rawNotes.trim() || !syllabusText.trim())}>{analyzeLoading ? '⏳ Analyzing…' : '🔍 Analyze Content'}</button>
                  <LoadingProgressBar loading={analyzeLoading} label="Analyzing Context" />{syllabusAnalysis && <p style={{ color: 'green', marginTop: 8 }}>✅ Syllabus mapped!</p>}{examAnalysis && <p style={{ color: 'green', marginTop: 8 }}>✅ Exam patterns analyzed!</p>}
                </form>
              </section>
            </ScrollReveal>
            <ScrollReveal isLocked={!uploadId}>
              <section style={translucentPanelStyle}>
                ...
                <form onSubmit={handleGenerateOutput}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 18, flexWrap: 'wrap' }}><label>Output Type: <select value={requestType} onChange={(e) => setRequestType(e.target.value)} style={glassySelectStyle}><option value="flashcards">📇 Flashcards</option><option value="study_plan">📋 Study Plan</option><option value="summary">📝 Summary</option><option value="mock_test">❓ Mock Test</option><option value="eli5">🧒 ELI5</option></select></label><input type="text" placeholder="Specific topic for ELI5 (optional)" value={specificTopic} onChange={(e) => setSpecificTopic(e.target.value)} style={{ ...glassyInputStyle, flex: 1 }} /></div>
                  <button type="submit" disabled={generateLoading || !uploadId} style={getActionButtonStyle(generateLoading || !uploadId)}>{generateLoading ? '⏳ Generating…' : '✨ Generate Output'}</button>
                  <LoadingProgressBar loading={generateLoading} label={`Generating ${requestType.replace('_', ' ')}`} />
                </form>
              </section>
            </ScrollReveal>
            {notice && <div style={{ color: '#000000', marginBottom: 24, padding: '16px 24px', backgroundColor: '#B3FF00', borderRadius: 20, fontWeight: 700, boxShadow: '0 10px 25px rgba(179, 255, 0, 0.2)' }}>{notice}</div>}
            {error && <div style={{ color: '#ff4d4d', marginBottom: 24, padding: '16px 24px', backgroundColor: 'rgba(255, 77, 77, 0.1)', border: '1px solid #ff4d4d', borderRadius: 20, fontWeight: 600 }}>❌ Error: {error}</div>}
            {generatedData && (
              <section className="fade-in" style={{ marginBottom: 60 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}><span style={{ fontSize: 24 }}>🧠</span><h2 style={{ margin: 0 }}>Visual Learning Lab</h2></div>
                {requestType === 'flashcards' && flashcards.length > 0 && <FlashcardSwiper cards={flashcards} onReachEndThreshold={fetchMoreFlashcards} apiBase={apiBase} />}
                {requestType === 'summary' && generatedData.keyTakeaways && <SummaryComponent data={generatedData} />}
                {requestType === 'eli5' && generatedData.simpleExplanation && <ELI5Component data={generatedData} />}
                {requestType === 'mock_test' && Array.isArray(generatedData.questions) && <MockTestComponent testData={generatedData} workspaceId={workspaceId} apiBase={apiBase} sessionId={sessionId} />}
                {requestType === 'study_plan' && generatedData.tasks && <StudyPlanComponent planData={generatedData} />}
              </section>
            )}
            <ScrollReveal isLocked={!uploadId}>
              <section style={translucentPanelStyle}>
                ...
                <form onSubmit={handleFetchHeatmap}>
                  <div style={{ display: 'flex', gap: 12, marginBottom: 12 }}><input type="text" value={workspaceId} onChange={(e) => setWorkspaceId(e.target.value)} placeholder="Workspace ID" style={{ ...glassyInputStyle, flex: 1 }} /><button type="submit" disabled={heatmapLoading || !workspaceId.trim()} style={getActionButtonStyle(heatmapLoading || !workspaceId.trim())}>{heatmapLoading ? '⏳ Fetching…' : '📈 Fetch Heatmap'}</button></div>
                  <LoadingProgressBar loading={heatmapLoading} label="Compiling Analytics" />
                </form>
                {heatmapError && <div style={{ color: '#ff4d4d', marginBottom: 12, padding: 16, backgroundColor: 'rgba(255, 77, 77, 0.1)', borderRadius: 20, border: '1px solid #ff4d4d', fontWeight: 600 }}>❌ Error: {heatmapError}</div>}
                {heatmapResult && (
                  <div style={{ marginTop: 24 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
                       <div style={{ padding: 24, backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: 30, border: '1px solid rgba(255,255,255,0.05)' }}><div style={{ color: accentColor, fontSize: 13, textTransform: 'uppercase', fontWeight: 800, marginBottom: 8 }}>Memory Retention</div><div style={{ fontSize: 36, fontWeight: 800 }}>{heatmapResult.overview.memoryRetentionRate}%</div></div>
                       <div style={{ padding: 24, backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: 30, border: '1px solid rgba(255,255,255,0.05)' }}><div style={{ color: accentColor, fontSize: 13, textTransform: 'uppercase', fontWeight: 800, marginBottom: 8 }}>Mastered Facts</div><div style={{ fontSize: 36, fontWeight: 800 }}>{heatmapResult.overview.totalCards} total</div></div>
                    </div>
                    <div style={{ padding: 24, backgroundColor: 'rgba(179, 255, 0, 0.08)', borderRadius: 30, border: `1px solid ${accentColor}`, marginBottom: 30 }}><div style={{ fontWeight: 800, color: accentColor, marginBottom: 10 }}>AI Action Plan</div><div style={{ lineHeight: 1.6 }}>{heatmapResult.overview.aiActionPlan}</div></div>
                    {Array.isArray(heatmapResult.heatmap) && heatmapResult.heatmap.length > 0 ? (
                      <div style={{ overflowX: 'auto', borderRadius: 24, border: '1px solid rgba(255,255,255,0.1)' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: 'rgba(34,34,34,0.6)' }}>
                          <thead><tr style={{ backgroundColor: 'rgba(0,0,0,0.4)' }}><th style={{ textAlign: 'left', padding: '16px 20px', color: accentColor, fontSize: 12, textTransform: 'uppercase' }}>Topic</th><th style={{ textAlign: 'left', padding: '16px 20px', color: accentColor, fontSize: 12, textTransform: 'uppercase' }}>Score</th><th style={{ textAlign: 'left', padding: '16px 20px', color: accentColor, fontSize: 12, textTransform: 'uppercase' }}>Status</th><th style={{ textAlign: 'left', padding: '16px 20px', color: accentColor, fontSize: 12, textTransform: 'uppercase' }}>Weak Concepts</th></tr></thead>
                          <tbody>{heatmapResult.heatmap.map((row, idx) => { const rowStatusStyle = heatmapStatusStyles[row.status] || { backgroundColor: '#f0f0f0', color: '#333' }; return (<tr key={`${row.topic}-${idx}`} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}><td style={{ padding: '16px 20px', fontWeight: 600 }}>{row.topic}</td><td style={{ padding: '16px 20px' }}>{row.avgScore}%</td><td style={{ padding: '16px 20px' }}><span style={{ padding: '6px 12px', borderRadius: 12, fontSize: 11, fontWeight: 800, ...rowStatusStyle, textTransform: 'uppercase' }}>{row.status}</span></td><td style={{ padding: '16px 20px', fontSize: 13, opacity: 0.7 }}>{row.missed || '—'}</td></tr>); })}</tbody>
                        </table>
                      </div>
                    ) : ( <div style={{ textAlign: 'center', padding: 40, opacity: 0.5 }}>No detailed topic data yet. Solve a test to see your heatmap.</div> )}
                  </div>
                )}
              </section>
            </ScrollReveal>
        ...
        {activeTab === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 30, flex: 1, padding: '20px 40px', boxSizing: 'border-box', overflow: 'hidden' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 30, height: '100%', overflowY: 'auto' }}>
                <div className="fade-in" style={{ backgroundColor: 'rgba(25, 25, 25, 0.7)', borderRadius: '40px', border: `5px solid ${isEmergencyActive ? '#ff4d4d' : 'rgba(179, 255, 0, 0.8)'}`, padding: '30px', backdropFilter: 'blur(25px)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, position: 'relative', overflow: 'hidden' }}>
                  ...
                  {!isVaultOpen && isSyncing && (<div style={{ zIndex: 12, textAlign: 'center', width: '80%', marginTop: 20 }}><div style={{ color: '#ff4d4d', fontWeight: 900, fontSize: 16, letterSpacing: 3, marginBottom: 10 }}>MISSION SYNCHRONIZATION</div><div className="sync-bar" style={{ height: 6 }}><div className="sync-bar-fill" /></div><div style={{ color: '#ff4d4d', fontSize: 10, marginTop: 10, opacity: 0.8, fontWeight: 700 }}>UPLOADING SURVIVAL PARAMETERS...</div></div>)}
                ...
                <div className="fade-in" style={{ backgroundColor: 'rgba(25, 25, 25, 0.7)', borderRadius: '40px', border: `5px solid ${isEmergencyActive ? '#ff4d4d' : 'rgba(179, 255, 0, 0.8)'}`, padding: '35px', backdropFilter: 'blur(25px)', lineHeight: 1.6, minHeight: 280 }}>
                  ...
                  {!isEmergencyActive && (<div style={{ fontSize: 14, color: '#B3FF00', fontWeight: 800, marginBottom: 20, padding: '10px 15px', border: '1px dashed #B3FF00', borderRadius: '15px', backgroundColor: 'rgba(179, 255, 0, 0.05)', textAlign: 'center' }}>⚡ ACTION REQUIRED: CLICK IGNIS ABOVE TO INITIALIZE SURVIVAL PROTOCOLS</div>)}
              ...
              <div className={`vault-container ${isVaultOpen ? 'vault-open' : ''}`} style={{ backgroundColor: 'rgba(10, 10, 10, 0.9)', borderRadius: '50px', border: `5px solid ${isEmergencyActive ? '#ff4d4d' : 'rgba(179, 255, 0, 0.8)'}`, display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
                <div className="vault-panel-corner corner-tl" /><div className="vault-panel-corner corner-tr" /><div className="vault-panel-corner corner-bl" /><div className="vault-panel-corner corner-br" />
            ...
            <div style={{ height: '40px', background: 'rgba(0,0,0,0.5)', borderTop: `1px solid ${isEmergencyActive ? '#ff4d4d' : 'rgba(179, 255, 0, 0.2)'}`, display: 'flex', alignItems: 'center', padding: '0 40px', gap: 30, overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><div style={{ width: 8, height: 8, borderRadius: '50%', backgroundColor: isEmergencyActive ? '#ff4d4d' : '#B3FF00' }} /><span style={{ fontSize: 10, fontWeight: 900, color: isEmergencyActive ? '#ff4d4d' : '#B3FF00' }}>SYSTEM STATUS: {isEmergencyActive ? 'COMBAT READY' : 'NOMINAL'}</span></div>
              <div style={{ flex: 1, overflow: 'hidden' }}><div style={{ fontSize: 9, fontFamily: 'monospace', opacity: 0.3, color: isEmergencyActive ? '#ff4d4d' : '#B3FF00' }}>[ ENCRYPTION_KEY: 0x9AF21 ] [ NEURAL_SYNC: 100% ] [ TRIAGE_PROTOCOL_V4 ] [ COMMANDER_SYNC: ACTIVE ]</div></div>
              <div style={{ fontSize: 10, fontWeight: 800, opacity: 0.5 }}>K.A.I. DEFENSE NETWORK © 2026</div>
            </div>
          </div>
        )}
      </div>
